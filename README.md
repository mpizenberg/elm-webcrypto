...
| `WebCrypto.KeyPair`     | ECDH P-256: `generateKeyPair`, `exportKeyPair`, `importKeyPair`, `publicKeyHash`, `deriveSharedKey`, `importPublicKey`                     |
...
**Opaque key handles.** `Key`, `KeyPair`, and `SigningKeyPair` are opaque types wrapping string IDs. The actual `CryptoKey` objects live in JS memory and are never serialized across the Elm/JS boundary.

**Pure Elm where possible.** String/JSON encryption and decryption, UTF-8 encoding, and all JSON codecs are pure Elm -- only the raw crypto operations call into JS.
